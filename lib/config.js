...
function saveSessionCommand(name, command) {
  const cfg = load();
  const history = (cfg.sessionHistory || []).map(s =>
    s.name === name ? { ...s, command } : s
  );
...
function saveWebPassword(password) {
  save({ webPassword: password || null });
}

function getWebPassword() {
  return load().webPassword || null;
}

module.exports = { load, saveTelegram, saveSessions, clearSessions, saveResumeCommand, addToHistory, removeFromHistory, getHistory, getAllSessionMeta, saveSessionMeta, saveSessionCommand, saveSetupPrefs, getSetupPrefs, saveWebPassword, getWebPassword };
