...
const { readJsonlMessages } = require('./ClaudeOutputParser');
...
class WebServer {
  constructor(manager, port = 3742, host = '127.0.0.1', password = null) {
    ...
    this._messageCache = new Map(); // name → messages[] for structured output
    this._sessionMeta = config.getAllSessionMeta(); // name → { emoji, color }
    this._usageCache = null; // { data, ts }
    this._logFile = path.join(os.homedir(), '.claude', 'pilot-web-debug.log');
    this._logReqCount = 0;
  }
  ...
  _buildAllSessions() {
    ...
    const offline = history
      ...
      .map(h => ({ name: h.name, path: h.path, command: h.command || 'claude', status: 'offline', startedAt: h.lastSeen, resumeAt: null }));
  ...
  // Parse raw tmux output into structured messages with incremental merging.
  // Cached per session so re-parsing only happens when output actually changes.
  _getMessages(name) {
    // Find the session's working directory to locate its JSONL file
    const session = this.manager.list().find(s => s.name === name);
    const cwd = session ? session.path : null;
    if (!cwd) {
      const hist = config.getHistory().find(h => h.name === name);
      if (!hist || !hist.path) return [];
      return readJsonlMessages(hist.path);
    }

    let cache = this._messageCache.get(name);
    if (!cache) { cache = {}; this._messageCache.set(name, cache); }
    return readJsonlMessages(cwd, { _cache: cache });
  }
  ...
  _detectMenuOptionsFromText(text) {
    return menuOptions.detect(text);
  }
  ...
  _handleApi(req, res, pathname, url) {
    ...
    if (req.method === 'GET' && outputMatch) {
      ...
      const includeMessages = url.searchParams.get('messages') !== '0'; // default on
      const result = { output: this._getOutput(name) };
      if (includeMessages) {
        result.messages = this._getMessages(name);
      }
    ...
    // PATCH /api/sessions/:name/command — update agent command (takes effect after respawn)
    const commandMatch = pathname.match(/^\/api\/sessions\/([^/]+)\/command$/);
    if (req.method === 'PATCH' && commandMatch) {
      const name = decodeURIComponent(commandMatch[1]);
      return this._readBody(req, (err, body) => {
        if (err) return this._json(res, 400, { error: err.message });
        const command = String(body.command || 'claude').trim();
        if (!command) return this._json(res, 400, { error: 'command required' });
        config.saveSessionCommand(name, command);
    ...
    // Path traversal guard: resolves rel inside cwd and returns null if it escapes
    const _safePath = (cwd, rel) => {
      const resolved = path.resolve(cwd, rel || '.');
      if (resolved !== cwd && !resolved.startsWith(cwd + path.sep)) return null;
      return resolved;
    };

    // GET /api/sessions/:name/files?path=<relpath>  — list directory
    const filesListMatch = pathname.match(/^\/api\/sessions\/([^/]+)\/files$/);
    if (req.method === 'GET' && filesListMatch) {
      const name = decodeURIComponent(filesListMatch[1]);
      const cwd = _sessPath(name);
      if (!cwd) return this._json(res, 404, { error: 'Session not found' });
      const rel = url.searchParams.get('path') || '.';
      const target = _safePath(cwd, rel);
      if (!target) return this._json(res, 400, { error: 'Invalid path' });
      try {
        const entries = fs.readdirSync(target, { withFileTypes: true })
          .map(e => {
            let size = null, mtime = null;
            try { const st = fs.statSync(path.join(target, e.name)); size = st.size; mtime = st.mtimeMs; } catch {}
        ...
        return this._json(res, 200, { entries, path: rel });
      } catch (e) {
        return this._json(res, 400, { error: e.message });
      }
    }

    // GET /api/sessions/:name/files/content?path=<relpath>  — read file
    const filesContentMatch = pathname.match(/^\/api\/sessions\/([^/]+)\/files\/content$/);
    if (req.method === 'GET' && filesContentMatch) {
      const name = decodeURIComponent(filesContentMatch[1]);
      const cwd = _sessPath(name);
      if (!cwd) return this._json(res, 404, { error: 'Session not found' });
      const rel = url.searchParams.get('path');
      if (!rel) return this._json(res, 400, { error: 'path required' });
      const target = _safePath(cwd, rel);
      if (!target) return this._json(res, 400, { error: 'Invalid path' });
      try {
        const stat = fs.statSync(target);
        if (stat.isDirectory()) return this._json(res, 400, { error: 'Path is a directory' });
        const MAX = 512 * 1024;
        if (stat.size > MAX) return this._json(res, 200, { tooBig: true, size: stat.size });
        const buf = fs.readFileSync(target);
        const isBinary = buf.slice(0, 8192).includes(0);
        if (isBinary) return this._json(res, 200, { binary: true, size: stat.size });
        return this._json(res, 200, { content: buf.toString('utf8'), size: stat.size });
      } catch (e) {
        return this._json(res, 400, { error: e.message });
      }
    }
  ...
  _broadcast() {
    try {
      ...
      if (ms > 200) this._log('SLOW', `broadcast blocked event loop ${ms}ms (${this._clients.size} SSE clients, ${this.manager.list().length} sessions)`);
    } catch (err) {
      this._log('ERR', `broadcast threw: ${err?.message ?? err}`);
    }
  }
  ...
  start() {
    ...
    this._log('INFO', `server starting on ${this.host}:${this.port} | log: ${this._logFile}`);

    this.server = http.createServer((req, res) => {
      const url = new URL(req.url, `http://127.0.0.1:${this.port}`);
      const pathname = url.pathname;
      const t0 = Date.now();
      const reqId = ++this._logReqCount;

      if (pathname !== '/events') {
        this._log('REQ', `#${reqId} ${req.method} ${pathname}`);
        res.on('finish', () => {
          const ms = Date.now() - t0;
          this._log('RES', `#${reqId} ${req.method} ${pathname} ${res.statusCode} ${ms}ms${ms > 500 ? ' *** SLOW ***' : ''}`);
        });
      }
      ...
      if (pathname === '/events') {
        if (!this._checkAuth(req, res, url)) return;
        // Chrome limits HTTP/1.1 to 6 concurrent connections per origin.
        // Cap SSE at 4 so 2 slots remain free for API requests.
        // Tabs beyond the cap get 503 and fall back to 5-second polling.
        const MAX_SSE = 4;
        if (this._clients.size >= MAX_SSE) {
          res.writeHead(503, { 'Retry-After': '10', 'Content-Type': 'text/plain' });
          res.end('Too many SSE clients — falling back to polling');
          this._log('SSE', `rejected new SSE client — at limit (${this._clients.size}/${MAX_SSE})`);
          return;
        }
        res.writeHead(200, {
        ...
        this._log('SSE', `client connected (total: ${this._clients.size})`);
        res.write(`data: ${JSON.stringify(this._buildAllSessions())}\n\n`);
        req.on('close', () => {
          this._clients.delete(res);
          this._log('SSE', `client disconnected (total: ${this._clients.size})`);
        });
        return;
      }
    ...
    this.server.on('error', (err) => this._log('ERR', `server error: ${err.message}`));
    ...
    this.server.listen(this.port, this.host, () => {
      this._log('INFO', `server listening on ${this.host}:${this.port}`);
      console.log(`  Debug log: ${this._logFile}`);
    });
    return this.port;
  }
  ...
  setPassword(pw) {
    ...
    this._token = this.password ? crypto.randomBytes(20).toString('hex') : null;
  }
