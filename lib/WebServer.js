...
class WebServer {
  ...
  // Runs claude in a hidden tmux window, sends /usage, captures the output, kills the window.
  // Returns { sessionPct, sessionReset, weekPct, weekReset } or null on failure.
  async _fetchQuota() {
    const sess = `pilot-quota-${process.pid}`;
    ...
    try {
      ...
      spawnSync('tmux', ['new-session', '-d', '-s', sess, '-x', '220', '-y', '50', claude], { timeout: 5000 });
      // Wait for claude to start (interactive prompt)
      ...
      const cap = spawnSync('tmux', ['capture-pane', '-t', sess, '-p', '-J'], { encoding: 'utf8', timeout: 3000 });
      const text = strip(cap.stdout || '');

      // Parse "Current session … XX% used\nResets RESET"
  ...
  _handleApi(req, res, pathname, url) {
    ...
    // GET /api/quota — session % and week % from claude /usage (tmux background fetch, 5-min cache)
    if (req.method === 'GET' && pathname === '/api/quota') {
      const QUOTA_TTL = 5 * 60 * 1000;
      const cached = this._quotaCache;
      // Return cached data immediately (even if stale — background refresh will update it)
      if (!this._quotaFetching && (!cached || Date.now() - cached.ts > QUOTA_TTL)) {
        this._quotaFetching = true;
        this._fetchQuota().then(data => {
          if (data) this._quotaCache = { data, ts: Date.now() };
        }).finally(() => { this._quotaFetching = false; });
      }
      return this._json(res, 200, cached ? cached.data : null);
    }
