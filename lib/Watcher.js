...
class Watcher {
  constructor(session, opts = {}) {
    ...
    this._busy = false;
    this._agentType = detectAgentType(session.command);
  ...
  _activeLimitWindow(text) {
    return this._stripAnsi(text)
      .split('\n')
      .filter(l => l.trim())
      .slice(-15)
      .join('\n');
  ...
  _hasActiveLimit(text) {
    return LIMIT_RE.test(this._activeLimitWindow(text));
  }
  ...
  _timeZoneOffsetMs(timeZone, date) {
    const p = this._timeZoneParts(date, timeZone);
    const wallAsUtc = Date.UTC(p.year, p.month - 1, p.day, p.hour, p.minute, p.second);
    return wallAsUtc - date.getTime();
  }

  _zonedTimeToMs(parts, timeZone) {
    const wallAsUtc = Date.UTC(parts.year, parts.month - 1, parts.day, parts.hour, parts.minute, 0, 0);
    let utcMs = wallAsUtc - this._timeZoneOffsetMs(timeZone, new Date(wallAsUtc));
    const correctedOffset = this._timeZoneOffsetMs(timeZone, new Date(utcMs));
    utcMs = wallAsUtc - correctedOffset;
    return utcMs;
  }

  _nextResetAtMs(hour, minute, timeZone) {
    const now = new Date();
    if (!timeZone) {
    ...
    try {
      const today = this._timeZoneParts(now, timeZone);
      let candidate = this._zonedTimeToMs({ ...today, hour, minute }, timeZone);
      if (candidate <= now.getTime()) {
        const nextDayUtc = Date.UTC(today.year, today.month - 1, today.day + 1, 12, 0, 0);
        const tomorrow = this._timeZoneParts(new Date(nextDayUtc), timeZone);
        candidate = this._zonedTimeToMs({ ...tomorrow, hour, minute }, timeZone);
      }
      return candidate;
    } catch {
      const resetAt = new Date(now);
  ...
  _parseResetAtMs(text) {
    ...
    if (atMatch) {
      ...
      if (m) {
        ...
        return this._nextResetAtMs(hour, minute, timeZone);
      }
    }
  ...
  async _check() {
    ...
    try {
      ...
      const limitWindow = this._activeLimitWindow(text);
  ...
  async _handleLimit(text) {
    ...
    try {
      ...
      if (this._hasActiveLimit(pane)) {
        ...
        this.lastHash = '';  // Reset hash to prevent re-detecting same limit message after blocking period
        return;
      }
    } catch { }
    ...
    this.lastHash = '';  // Reset hash to prevent re-detecting old limit messages in scrollback
