...
class WebServer {
  constructor(manager, port = 3742, host = '127.0.0.1', password = null) {
    ...
    this._quotaCache = null; // { data, ts }
    this._quotaPromise = null; // shared in-flight fetch promise
  }
  ...
  // Runs claude in a hidden tmux session, sends /usage, polls until output appears, kills session.
  // Returns { sessionPct, sessionReset, weekPct, weekReset } or null on failure.
  async _fetchQuota() {
    const sess = `pilot-quota-${Date.now()}`;
    const strip = s => s.replace(/\x1b\[[0-9;]*[A-Za-z]/g, '').replace(/\r/g, '');
    const capture = () => {
      const r = spawnSync('tmux', ['capture-pane', '-t', sess, '-p', '-J'], { encoding: 'utf8', timeout: 3000 });
      return strip(r.stdout || '');
    };
    const pollFor = async (needle, intervalMs, maxMs) => {
      const deadline = Date.now() + maxMs;
      while (Date.now() < deadline) {
        await new Promise(r => setTimeout(r, intervalMs));
        if (capture().includes(needle)) return true;
      }
      return false;
    };
    try {
      const claude = process.env.CLAUDE_PATH || 'claude';
      // Kill any lingering session with this name before starting
      spawnSync('tmux', ['kill-session', '-t', sess], { timeout: 2000 });
      // --dangerously-skip-permissions bypasses the workspace trust dialog shown
      // when claude starts in an untrusted directory (e.g. the server's cwd = $HOME)
      spawnSync('tmux', ['new-session', '-d', '-s', sess, '-x', '220', '-y', '50',
        claude, '--dangerously-skip-permissions'], { timeout: 5000 });
      // Wait until the claude prompt appears (≤ 10 s)
      const ready = await pollFor('? for shortcuts', 500, 10000);
      if (!ready) return null;
      spawnSync('tmux', ['send-keys', '-t', sess, '/usage', 'Enter'], { timeout: 2000 });
      // Wait until the usage output appears (≤ 6 s)
      const rendered = await pollFor('Current session', 500, 6000);
      if (!rendered) return null;
      const text = capture();

      const sessBlock = text.match(/Current session[\s\S]*?(\d+)%\s+used[^\n]*\n([^\n]*)/);
      const weekBlock = text.match(/Current week[\s\S]*?(\d+)%\s+used[^\n]*\n([^\n]*)/);
      const parseReset = s => s ? s.trim().replace(/^\s*Resets\s+/i, '').trim() : null;

      const sessionPct   = sessBlock ? parseInt(sessBlock[1]) : null;
      const sessionReset = sessBlock ? parseReset(sessBlock[2]) : null;
      const weekPct      = weekBlock ? parseInt(weekBlock[1]) : null;
      const weekReset    = weekBlock ? parseReset(weekBlock[2]) : null;

      if (sessionPct === null && weekPct === null) return null;
      return { sessionPct, sessionReset, weekPct, weekReset };
    } catch {
      return null;
    } finally {
      spawnSync('tmux', ['kill-session', '-t', sess], { timeout: 2000 });
  ...
  _handleApi(req, res, pathname, url) {
    ...
    // GET /api/quota — session % and week % from claude /usage (tmux fetch, 5-min cache)
    if (req.method === 'GET' && pathname === '/api/quota') {
      const QUOTA_TTL = 5 * 60 * 1000;
      const cached = this._quotaCache;
      if (cached && Date.now() - cached.ts < QUOTA_TTL) {
        return this._json(res, 200, cached.data);
      }
      // No valid cache — start or join an in-flight fetch, wait for result
      if (!this._quotaPromise) {
        this._quotaPromise = this._fetchQuota().then(data => {
          if (data) this._quotaCache = { data, ts: Date.now() };
          return data;
        }).finally(() => { this._quotaPromise = null; });
      }
      this._quotaPromise.then(data => this._json(res, 200, data)).catch(() => this._json(res, 200, null));
      return;
    }
